	    ...
	    Users.forEach(function(user){//所有已連線使用者
	        if(USE_MESSAGE){
    	        ...
    	        getNewMessage(user.userNO).then(function(data){
    	            if(data.length>0) {
    	                updateMessageStatus(data[0].NO,{userNO:-1,IP:user.socket.handshake.headers["x-forwarded-for"]});
 ...
 **/
function updateMessageStatus(MessageNO,userData){
    let DB = new Sql.DB();
    ...
    ],"Message",userData,6).then(function(){
        console.log("susses");
    },function(err){
        console.log(err);
    });
