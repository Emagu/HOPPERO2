...
function register(userData,path){
	return new Promise(function(success,fail){
		...
		let link = Tool.encodeURL(path + "/verify",{id: hash});
		let mailOptions = {
 ...
 * @param {string} path : 伺服器網址
 **/
function reSetPassword(userData,path){
	return new Promise(function(success,fail){
		let date = Tool.getTimeZone();
		let certification = '2';
		let hash = crypto.createHash('sha256').update(userData.account+date+userData.mail+certification).digest('base64');
		...
		let mailOptions = {
			to: userData.mail, //收件者
			subject: '密碼重置-申請信', // 主旨
			html: "請點擊下列連接重置密碼<br><a href="+link+">"+link+"</a>"
		};
		CreateVerify(hash,userData.userNO,"2",AccountConfig.ReSetPasswordVerifyTime,userData.IP).then(function(){
			// 送信
			transporter.sendMail(mailOptions, (error, info) => {
				if (error) {
					console.log(error);
